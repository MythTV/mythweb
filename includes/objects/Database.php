 ...
 *                  a read-only context.
/**/
    public $limit_regex = '/((.*)\sLIMIT\s+\d+(?:\s*(?:,|OFFSET)\s*\d+)?)?
                           ((?:\s+PROCEDURE\s+\w+\(.+?\))?
                            (?:\s+FOR\s+UPDATE)?
...
/**/
    public $limit_regex_replace = '"$2"." LIMIT 1 "."$3"';
...
/**/
    public function &connect($db_name, $login, $password, $server='localhost', $port=NULL, $engine='mysql_detect', $options=array()) {
...
/**/
    public function smart_args($args) {
        $new_args = array();
    // Not an array
        if (!is_array($args))
            return array($args);
