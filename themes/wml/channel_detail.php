...
class Theme_channel_detail extends Theme {
    ...
    function print_page() {
        ...
        global $this_channel;
?>
<p><br/><a href="#cardjump"><?php echo _LANG_JUMP_TO." "._LANG_DATE?></a><br/>
Channel <?php echo $this_channel->channum?> <?php echo $this_channel->callsign?></p>
<p>
<?php 
        if (isset($_SESSION['list_time'])) {
            echo strftime(generic_date, $_SESSION['list_time']);
        }
?>
</p>
<p>
<?php
        ...
        for ($i=-1;$i<=$max_days;$i++) {
            ...
            $date = date("Ymd", $time);
            $carddata.="<a href=\"channel_detail.php?chanid=".$_GET['chanid']."&amp;time=".$time."\">".date("m/d", $time)."</a>\n";
        }
...
<?php
        ...
        foreach ($this_channel->programs as $show) {
            // Print the content
            echo '<option value="'.$show->starttime.'">'.htmlspecialchars($show->title)." (".date('g:i A', $show->starttime).")</option>\n";
        }
        echo '</select></p></card>';
        echo '<card id="cardjump"><p>';
        echo $carddata;
        echo '</p></card>';
    }
}
